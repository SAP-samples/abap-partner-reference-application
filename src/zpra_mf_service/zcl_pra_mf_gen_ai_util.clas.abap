CLASS zcl_pra_mf_gen_ai_util DEFINITION
  ...
  PRIVATE SECTION.
    ...
    METHODS get_sys_prompt_with_rhyme
      RETURNING
        VALUE(result) TYPE string
      RAISING
        cx_aic_api_factory
    ...
    METHODS get_mf_prompt
     ...
     METHODS get_mf_prompt_with_params
      IMPORTING
        template_id   TYPE aic_islm_prompt_template_id=>type
        parameters    TYPE if_aic_prompt_template=>param_values
      RETURNING
        VALUE(result) TYPE string
      RAISING
        cx_aic_api_factory
...
CLASS ZCL_PRA_MF_GEN_AI_UTIL IMPLEMENTATION.
  ...
  METHOD zif_pra_mf_gen_ai_util~generate_music_festival_data.
    ...
    DATA(system_prompt) = COND #(
      WHEN rhyme_indicator = abap_true
      THEN get_sys_prompt_with_rhyme( )
      ELSE get_sys_prompt_base( ) ).
    messages->set_system_role( system_prompt ).
  ...
  METHOD get_mf_data_gen_user_prompt.
    ...
    lang_prompt = get_mf_prompt_with_params( template_id = set_language_system_prompt-template_name
  ...
  METHOD get_mf_prompt_with_params.
  ...
  METHOD get_sys_prompt_with_rhyme.
    DATA(base_prompt)  = get_mf_prompt( template_id = desc_system_prompt-template_name ).
    DATA(rhyme_prompt) = get_mf_prompt( template_id = add_rhyme_system_prompt-template_name ).
    result = | { base_prompt } { cl_abap_char_utilities=>newline } { rhyme_prompt } |.
  ENDMETHOD.
ENDCLASS.
