CLASS zcl_pra_mf_ent_proj_outb_integ DEFINITION
  ...
  PUBLIC SECTION.
    ...
    INTERFACES zif_pra_mf_ent_proj_integ.
ENDCLASS.
...
CLASS ZCL_PRA_MF_ENT_PROJ_OUTB_INTEG IMPLEMENTATION.
  ...
  METHOD zif_pra_mf_ent_proj_integ~create_entproject.
    TYPES BEGIN OF ty_business_details.
            INCLUDE TYPE zcl_pra_mf_scm_ent_proj=>tys_a_enterprise_project_type.
    TYPES   to_enterprise_project_el_2 TYPE zcl_pra_mf_scm_ent_proj=>tyt_a_enterprise_project_ele_2.
    TYPES END OF ty_business_details.

    DATA header_properties TYPE TABLE OF string.
    DATA business_details  TYPE ty_business_details.
    DATA request           TYPE REF TO /iwbep/if_cp_request_create.
    DATA response          TYPE REF TO /iwbep/if_cp_response_create.
    DATA child_properties  TYPE TABLE OF string.
...
*        request = get_clientproxy( )->create_resource_for_entity_set( 'A_ENTERPRISE_PROJECT' )->create_request_for_create( ).
        DATA(proxy_result) = zif_pra_mf_ent_proj_integ~get_clientproxy( ).
          DATA(client_proxy) = proxy_result-client_proxy.
      ...
      CATCH /iwbep/cx_gateway INTO gateway_error.
        result-messages = VALUE #( ( type = 'E' id = 'ZPRA_MF_MSG_CLS' number = '009' ) ).
    ENDTRY.
  ...
  METHOD zif_pra_mf_ent_proj_integ~get_clientproxy.
    ...
    TRY.
        ...
        " create client proxy
        result-client_proxy = /iwbep/cl_cp_factory_remote=>create_v2_remote_proxy(
      ...
      CATCH cx_http_dest_provider_error INTO DATA(provider_error). " TODO: variable is assigned but never used (ABAP cleaner)
        result-messages = VALUE #( ( type = 'E' id = 'ZPRA_MF_MSG_CLS' number = '009' ) ).
      CATCH /iwbep/cx_gateway INTO DATA(cx_gateway_error). " TODO: variable is assigned but never used (ABAP cleaner)
        result-messages = VALUE #( ( type = 'E' id = 'ZPRA_MF_MSG_CLS' number = '009' ) ).
      CATCH cx_web_http_client_error INTO DATA(http_client_error). " TODO: variable is assigned but never used (ABAP cleaner)
        result-messages = VALUE #( ( type = 'E' id = 'ZPRA_MF_MSG_CLS' number = '009' ) ).
    ENDTRY.
  ENDMETHOD.
ENDCLASS.
